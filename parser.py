...
def parse_args(default_config_path):
    parser = argparse.ArgumentParser(
        description="Cleanup script, can detect all sorts of unwanted files. "
                    "All options have choices 'y' (yes), 'n' (no), 'ask' (prompt)."
    )
    parser.add_argument(
        "-x", "--main-dir", 
        help="Directory to store all files.", 
        required=True
    )
    parser.add_argument(
        "-d", "--dir", 
        nargs='*', 
        help="Additional directories to be cleaned up. Can be specified multiple times."
    )
    parser.add_argument(
        "--config-file", "--config",
        type=str,
        default=default_config_path,
        help="Path to custom configuration file (default: {}).".format(default_config_path)
    )
    parser.add_argument(
